...
interface TableContents {
  tableHeaders: string[];
  tableDatas: ExceptionEventDto[];
  tableKeys: string[];
  specialDisplay?: TableSpecialDisplay<ExceptionEventDto>[];
  onClick:(data:ExceptionEventDto) => void;
}
...
const CommandStatusTable: React.FC<PropsWithChildren<TableContents>> = ({
  ...
  const sortedDatas = useMemo(() => {
    ...
    data.sort((a, b) => {
      const aValue = normalizedValue(a[sortKey as keyof ExceptionEventDto]);
      const bValue = normalizedValue(b[sortKey as keyof ExceptionEventDto]);
      ...
      <div className="max-h-[70vh] overflow-auto scrollbar-thin scrollbar-transparent">
        <Table className="w-full table-fixed">
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            ...
            {sortedDatas &&
              sortedDatas.map((data: ExceptionEventDto, i: number) => (
                <TableRow
                onClickWithEvent={() => {
                  console.log(data)
                  onClick(data)
