...
export const DeviceEndpoint = {
  ...
  GET_READER: (moduleId: number) =>
    `/api/${CONTROLLER}/module/reader/options/${moduleId}`,
  GET_INPUT: (moduleId: number) =>
    `/api/${CONTROLLER}/module/input/options/${moduleId}`,
  GET_RELAY: (moduleId: number) =>
    `/api/${CONTROLLER}/module/relay/options/${moduleId}`,
  CHECK_AMICO_CONNECT: `/api/${CONTROLLER}/${Vendor.amico}/connect`,
} as const;
