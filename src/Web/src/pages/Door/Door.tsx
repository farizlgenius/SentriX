    ...
    useEffect(() => {
        ...
        connection.on(
            ...
            (status: AcrStatus) => {
                setStatus((prev) =>
                    prev.map((a) =>
                        a.deviceComponentId == status.scpId && a.componentId == status.number ? {
                            ...a,
        ...
        return [
            <>
                <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    <>
                        <Badge size="sm" color="dark">{statusDto.find(b => b.deviceComponentId == data.scpId)?.tamper}</Badge>
                    </>
                </TableCell>
                <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    <>
                        {statusDto.find(b => b.deviceComponentId == data.scpId)?.status === "Secure" ? (
                            <Badge size="sm" color="success">{statusDto.find(b => b.deviceComponentId == data.scpId)?.status}</Badge>
                        ) : statusDto.find(b => b.deviceComponentId == data.scpId)?.status === "Forced Open" || statusDto.find(b => b.deviceComponentId == data.scpId)?.status === "Locked" ? (
                            <Badge size="sm" color="error">{statusDto.find(b => b.deviceComponentId == data.scpId)?.status}</Badge>
                        ) : (
                            <Badge size="sm" color="warning">{statusDto.find(b => b.deviceComponentId == data.scpId)?.status === 0 ? "Error" : statusDto.find(b => b.deviceComponentId == data.scpId)?.status}</Badge>
                        )}
                    </>
                </TableCell>
            </>
