      ...
      <>
        <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
          <Badge
            size="sm"
            color={
              statusDto.find(b => b.componentId == data.deviceId)?.tamper == "Active"
                ? "success"
                : statusDto.find(b => b.componentId == data.deviceId)?.tamper == "Inactive"
                  ? "error"
                  : "warning"
            }
          >
            {statusDto.find(b => b.componentId == data.deviceId)?.tamper == "" ? "Error" : statusDto.find(b => b.componentId == data.deviceId)?.tamper}
          </Badge>
        </TableCell>
        <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
          <Badge
            size="sm"
            color={
              statusDto.find(b => b.componentId == data.deviceId)?.ac == "Active"
                ? "success"
                : statusDto.find(b => b.componentId == data.deviceId)?.ac == "Inactive"
                  ? "error"
                  : "warning"
            }
          >
            {statusDto.find(b => b.componentId == data.deviceId)?.ac == "" ? "Error" : statusDto.find(b => b.componentId == data.deviceId)?.ac}
          </Badge>
        </TableCell>
        <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
          <Badge
            size="sm"
            color={
              statusDto.find(b => b.componentId == data.deviceId)?.batt == "Active"
                ? "success"
                : statusDto.find(b => b.componentId == data.deviceId)?.batt == "Inactive"
                  ? "error"
                  : "warning"
            }
          >
            {statusDto.find(b => b.componentId == data.deviceId)?.batt == "" ? "Error" : statusDto.find(b => b.componentId == data.deviceId)?.batt}
          </Badge>
        </TableCell>
        <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
          <Badge
            size="sm"
            color={
              statusDto.find(b => b.componentId == data.deviceId)?.status == "Online" || statusDto.find(b => b.componentId == data.deviceId)?.status == "Active"
                ? "success"
                : statusDto.find(b => b.componentId == data.deviceId)?.status == "Offline"
                  ? "error"
                  : "warning"
            }
          >
            {statusDto.find(b => b.componentId == data.deviceId)?.status == "" ? "Offline" : statusDto.find(b => b.componentId == data.deviceId)?.status}
          </Badge>
        </TableCell>
      </>
    ]
