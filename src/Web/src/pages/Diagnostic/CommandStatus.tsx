...
import { AlertIcon, AmicoIcon, CalenderIcon, CancelCircleIcon, CheckCircleIcon, ControlIcon, DoorIcon, HardwareIcon, ModuleIcon, MonitorIcon, TimeIcon, TimezonIcon, UserIcon } from '../../icons'
...
const headers: string[] = [
  "Send","Name","Mac","Command","Response","Reason","Status"
]
...
const CommandStatus = () => {
  ...
  ({
    [DeviceType.AERO.toString()]: <ModuleIcon className="w-6 h-6" style={{ color: accentColor }} />,
    [DeviceType.AMICO.toString()]: <AmicoIcon className="w-6 h-6" style={{ color: accentColor }} />
  }[mod] ?? <ModuleIcon className="w-6 h-6" style={{ color: accentColor }} />);
  ...
  async function fetchData(pageNumber: number, pageSize: number, search?: string, startDate?: string, endDate?: string) {
    const res = await send.get(EventEndpoint.GET_COMMAND_PAGINATION(pageNumber, pageSize, locationId, search, startDate, endDate));
    if (res && res.data) {
      ...
      <div className="space-y-6">
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
          <div className="max-w-full overflow-x-auto">
            ...
            <CommandStatusTable tableHeaders={headers} tableDatas={tableDatas} tableKeys={keys} specialDisplay={[
              {
                key: "name",
              ...
              },
              {
                ...
                content: (data, i) => <TableCell key={i} className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                  <span className='flex gap-2'>
                   ...
                   {new Intl.DateTimeFormat("en-GB").format(new Date(data.send))}  {new Date(data.send).toTimeString().split(" ")[0]}
                  </span>
              ...
              },
               {
                ...
                content: (data, i) => <TableCell key={i} className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                  <span className='flex gap-2'>
                    {<CalenderIcon className="w-5 h-5" style={{color:accentColor}} />}
                   {new Intl.DateTimeFormat("en-GB").format(new Date(data.received))}  {new Date(data.received).toTimeString().split(" ")[0]}
                  </span>
                  ...
                  <span className='flex gap-2'>
                    {
                        data.status == EventCommandStatus.SUCCESSED ? 
                        <CheckCircleIcon className="w-5 h-5" style={{color:'green'}} />
                        :
                        data.status == EventCommandStatus.FAILED ?
                        <CancelCircleIcon className="w-5 h-5" style={{color:'red'}} />
                        :
                        <AlertIcon className="w-5 h-5" style={{color:accentColor}} />
                        }
                  </span>
