...
import ExceptionEventTable from "../../components/tables/Tables/ExceptionEventTable";
import { ExceptionEventDto } from "../../model/Event/ExceptionEventDto";
...
const defaultEx:ExceptionEventDto = {
      timestamp: new Date(),
      path: "",
      exception: "",
      innerException: "",
...
const ExceptionEvent = () => {
  ...
  const handlePageSizeSelect = (data: string) => {
   ...
   const handleClick = (item: ExceptionEventDto) => {
    ...
    const [exception,setException] = useState<ExceptionEventDto>(defaultEx);
  ...
  const [tableDatas, setTablesData] = useState<ExceptionEventDto[]>([]);
  async function fetchData(
    ...
    const res = await send.get(
      EventEndpoint.GET_EXCEPTION_PAGINATION(
        pageNumber,
        pageSize,
        locationGuid,
        search,
  ...
  useEffect(() => {
    ...
    return () => {
      const connection = SignalRService.getConnection();
      connection?.off(SignalRTopic.EXCEPTION_EVENT);
    };
  }, [locationGuid]);
  ...
  return (
    <>
      ...
      {
            popup && (
                  <Modals header="Exception Detail" handleClickWithEvent={modalHandleClickWithEvent} isWide={true} body={
                  <FormSection className="grid grid-cols-3 gap-4">
                        <FormField>
                              <Label>Exception</Label>
                              <TextArea className="h-100" value={exception.exception} />
                        </FormField>
                         <FormField>
                              <Label>Inner Exception</Label>
                              <TextArea className="h-100" value={exception.innerException} />
                        </FormField>
                         <FormField>
                              <Label>Stack Trace</Label>
                              <TextArea className="h-100" value={exception.stackTrace} />
                        </FormField>
                  </FormSection>}  />
            )
      }
