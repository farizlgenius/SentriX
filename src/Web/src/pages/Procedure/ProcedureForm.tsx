...
export const ProcedureForm: React.FC<
  PropsWithChildren<FormProp<ProcedureDto>>
> = ({ handleClick, dto, setDto, type }) => {
    ...
    switch (e.target.name) {
      case "actionType":
        setAction((prev) => ({
          ...
          actionTypeDesc:
            actionType.find((a) => a.value == Number(value))?.label ?? "",
        }));
        switch (Number(value)) {
          case 1:
            fetchMp();
      ...
      case "arg1":
        ...
        const mac =
          mp.find((x) => x.value === Number(value))?.description ?? "";
  ...
  const fetchCp = async () => {
    var res = await api.get(OutputEndpoint.GET(locationId));
    if (res && res.data.data) {
        ...
        <>
          ...
          <div className="flex gap-4">
            <Button
              disabled={type == FormType.INFO}
              className="flex-1"
              name={type == FormType.CREATE ? "create" : "update"}
              onClick={handleClick}
              variant="primary"
            >
