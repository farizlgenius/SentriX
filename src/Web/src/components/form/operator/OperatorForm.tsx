...
> = ({ handleClick: handleClickWithEvent, dto, setDto, type }) => {
  ...
  const fetchLocation = async () => {
    ...
    if (res?.data) {
      setLocations(
        res.data.map((location: LocationDto) => ({
          label: location.name,
          value: location.id,
          isTaken: false,
        })),
  ...
  const addLocation = () => {
    ...
    setDto((prev) => ({
      ...prev,
      locationId: [...prev.locationId, locationsId],
    }));
  ...
  const removeSelectedLocations = () => {
    ...
    setDto((prev) => ({
      ...prev,
      locationId: prev.locationId.filter((id) => !idsToRemove.includes(id)),
    }));
    setLocations((prev) =>
      prev.map((option) =>
        idsToRemove.includes(Number(option.value))
  ...
  const capturePhoto = () => {
    ...
    canvas.toBlob((blob) => {
      if (!blob) return;
      const file = new File([blob], `operator_${Date.now()}.png`, {
        type: "image/png",
      });
      applyImage(file);
      setCameraOpen(false);
        ...
        <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
          <aside className="rounded-[28px] border border-[var(--app-panel-border)] bg-[var(--app-panel-bg)] p-6 shadow-theme-xs">
            <div className="rounded-[24px] bg-[linear-gradient(180deg,rgba(59,130,246,0.10),rgba(255,255,255,0))] p-5 dark:bg-[linear-gradient(180deg,rgba(59,130,246,0.18),rgba(17,24,39,0))]">
              ...
              <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                Add an operator photo by dragging a file, browsing your device,
                or capturing one from the webcam.
              </p>
            </div>
            ...
            <div className="mt-6 flex flex-col items-center gap-4">
              ...
              >
                ...
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  PNG, JPG, or WebP. This preview is local until an upload API
                  is connected.
                </p>
              </div>
          ...
          <section className="space-y-6">
                  ...
                  <Select
                    ...
                    onChange={(value) =>
                      setDto((prev) => ({
                        ...
                        roleId: Number(value),
                        role:
                          roles.find((option) => option.value === Number(value))
                            ?.label ?? "",
                      }))
                    }
                    name="roleId"
            ...
            <div className="rounded-[28px] border border-[var(--app-panel-border)] bg-[var(--app-panel-bg)] p-6 shadow-theme-xs lg:p-8">
              <div className="mb-6">
                ...
                <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                  Add locations one by one, then tap cards below to mark which
              ...
              <div className="flex flex-col gap-3 lg:flex-row">
                ...
                <div className="flex gap-3 lg:items-end">
                  <Button
                    disabled={isReadOnly || locationsId === -1}
                    onClick={addLocation}
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {dto.locationId.length > 0 ? (
                  dto.locationId.map((id) => (
                    <button
                      key={id}
                      type="button"
