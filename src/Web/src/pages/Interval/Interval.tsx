...
export const INTERVAL_TABLE_HEAD: string[] = [
  "Start Time",
  "End Time",
  "Days",
  "Action",
...
const Interval = () => {
    ...
    switch (e.currentTarget.name) {
      ...
      case "delete":
        ...
        setConfirmRemove(() => async () => {
          ...
          const res = await send.post(IntervalEndpoint.DELETE_RANGE, data);
          if (
            Helper.handleToastByResCode(
              res,
              IntervalToast.DELETE_RANGE,
              toggleToast,
            )
          ) {
            setRemove(false);
      ...
      case "create":
        ...
        intervalDto.locationGuid = locationGuid;
        setConfirmCreate(() => async () => {
          if (!Helper.isDayEmpty(intervalDto.days)) {
            toggleToast("error", "Day can't be empty");
          } else if (
            !Helper.isValidTimeRange(intervalDto.start, intervalDto.end)
          ) {
  ...
  const handleRemove = (data: IntervalDto) => {
    setConfirmRemove(() => async () => {
      const res = await send.delete(IntervalEndpoint.DELETE(data.guid));
  ...
  const fetchData = async (
    ...
    if (res.data.success) {
      setIntervalsDto(res.data.data.items);
    ...
    <>
      ...
      {form ? (
        ...
        <div className="space-y-6">
          <BaseTable<IntervalDto>
            headers={INTERVAL_TABLE_HEAD}
            keys={INTERVAL_KEY}
            data={intervalsDto}
                ...
                content: (d, i) => (
                  ...
                  >
                    <div className="flex gap-5">
                      <p
                        className={
                          d.days.sunday ? "text-green-500" : "text-gray-700"
                        }
                      >
                        Sun
                      </p>
                      <p
                        className={
                          d.days.monday ? "text-green-500" : "text-gray-700"
                        }
                      >
                        Mon
                      </p>
                      <p
                        className={
                          d.days.tuesday ? "text-green-500" : "text-gray-700"
                        }
                      >
                        Tue
                      </p>
                      <p
                        className={
                          d.days.wednesday ? "text-green-500" : "text-gray-700"
                        }
                      >
                        Wed
                      </p>
                      <p
                        className={
                          d.days.thursday ? "text-green-500" : "text-gray-700"
                        }
                      >
                        Thu
                      </p>
                      <p
                        className={
                          d.days.friday ? "text-green-500" : "text-gray-700"
                        }
                      >
                        Fri
                      </p>
                      <p
                        className={
                          d.days.saturday ? "text-green-500" : "text-gray-700"
                        }
                      >
                        Sat
                      </p>
