...
public sealed class DeviceRepository(DeviceDbContext context) : IDeviceRepository
{
      ...
      public async Task<IEnumerable<OptionDto>> GetOptionByLocationIdTypeAeroAsync(int locationId,string type, CancellationToken ct = default)
      {
            ...
            .Select(x => new OptionDto(x.name, 0,x.mac,x.guid,false))
            .ToArrayAsync();
      }
      ...
      public async Task<IEnumerable<(Guid Guid,string Type)>> GetGuidAndTypesByLocationIdAsync(int LocationId,CancellationToken ct = default)
      {
            var list = await context.Devices.AsNoTracking()
            .Where(x => x.location_id == LocationId)
                  .Select(x => new { x.guid,x.type })
                  .ToListAsync(ct);

            return list.Select(x => (x.guid,x.type));
