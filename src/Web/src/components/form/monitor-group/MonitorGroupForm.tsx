...
export const MonitorGroupForm: React.FC<
  PropsWithChildren<FormProp<MonitorGroupDto>>
> = ({ handleClick, dto, setDto, type }) => {
  ...
  const handleSelect = (
    ...
    switch (e.target.name) {
      ...
      case "pointType":
        ...
        setMp((prev) => ({
          ...prev,
          pointType: Number(value),
          pointTypeDesc:
            typeOptions.find((x) => x.value == Number(value))?.label ?? "",
        }));
        switch (Number(value)) {
          case 1:
  ...
  const handleClickAdd = () => {
    setDto((prev) => ({
      ...prev,
      nMpList: prev.nMpList.includes(mp)
        ? [...prev.nMpList]
        : [...prev.nMpList, mp],
    }));
    setMp(defaultDto);
      ...
      <div className="flex flex-col gap-5 justify-center items-center p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
        ...
        <div className="flex gap-2 w-1/2">
          ...
          <div className="flex justify-end items-end">
            <Button name="add" onClick={handleClickAdd}>
              Add
            </Button>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-5 justify-center items-center p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
            <div className="grid grid-cols-5 gap-4">
              {dto.nMpList.map((s, i) => (
                <div
                  key={i}
                  className={`cursor-pointer flex flex-col rounded-2xl border border-gray-200 hover:dark:bg-white/[0.01] hover:bg-gray-200 ${selected.includes(s) ? "bg-gray-200 dark:bg-white/[0.01]" : "dark:bg-white/[0.03] bg-white"} p-5 dark:border-gray-800  md:p-6`}
                >
